...
?>
		...
		mode: '<? echo $mode; ?>',
		infiniteLoop: <? echo $infiniteLoop; ?>,
		speed: <? echo $speed; ?>,
		randomStart: <? echo $randomStart; ?>,
		adaptiveHeight: <? echo $adaptiveHeight; ?>,
		adaptiveHeightSpeed: <? echo $adaptiveHeightSpeed; ?>,
		pager: <? echo $pager; ?>,
		pagerType: '<? echo $pagerType; ?>',
		controls: <? echo $controls; ?>,
		nextText: '<? echo $nextText; ?>',
		prevText: '<? echo $prevText; ?>',
		autoControls: <? echo $autoControls; ?>,
		startText: '<? echo $startText; ?>',
		stopText: '<? echo $stopText; ?>',
		autoControlsCombine: <? echo $autoControlsCombine; ?>,
		auto: <? echo $auto; ?>,
		pause: <? echo $pause; ?>,
		autoStart: <? echo $autoStart; ?>,
		autoDirection: '<? echo $autoDirection; ?>',
		autoHover: <? echo $autoHover; ?>,
		autoDelay: <? echo $autoDelay; ?>,
		moveSlides: <? echo $moveSlides; ?>
   				
   				}" 
   				data-breaks="[{screen:0, slides:<? echo $bp1; ?>, pager:false},{screen:460, slides:<? echo $bp2; ?>},{screen:768, slides:<? echo $bp3; ?>}]" >
   				<?php
				...
				<?php if ( isset( $instance['show_readmore'] ) ) : ?>
					<div class="ups_rm"><a href="<?php the_permalink(); ?>"><?php echo $instance[ 'readmore_text' ]; ?></a></div>
				<?php endif; ?>
				
				<?php 
					if ($instance['show_date'] === 'hide') {
        			?>
        				<p>date hidden here</p>
        			<?
   					}
   					elseif ($instance['show_date'] === 'created') {
   					?>
   						<p><?php the_time($date_format);?></p>
   					<?
    				}
    				
    				elseif ($instance['show_date'] === 'modified') {
   					?>
